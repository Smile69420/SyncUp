...
import type { BookingDetails, EventType, MergedBooking } from '../types';
...
const BookingDetailsEditorModal: React.FC<EditorProps> = ({ booking, eventType, onClose, onSave }) => {
        ...
        <Modal title="Edit Booking Details" onClose={onClose}>
            <div className="space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar pr-4">
                 <fieldset className="p-4 border rounded-lg bg-slate-50">
                    <legend className="px-2 font-semibold text-slate-700">Client Details</legend>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-500">Name</label>
                            <p className="mt-1 font-semibold text-slate-800">{booking.bookerName}</p>
                        </div>
                        <div>
                         ...
                         <div>
                            ...
                            <p className="mt-1 font-semibold text-slate-800">{booking.bookerEmail}</p>
                        </div>
                         <div>
                            <label className="block text-sm font-medium text-slate-500">Phone</label>
                            <p className="mt-1 font-semibold text-slate-800">{booking.bookerPhone}</p>
                        </div>
                    </div>
                </fieldset>

                 {booking.customAnswers && Object.keys(booking.customAnswers).length > 0 && eventType && (
                     <fieldset className="p-4 border rounded-lg bg-slate-50">
                        <legend className="px-2 font-semibold text-slate-700">Submitted Answers</legend>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                             {Object.entries(booking.customAnswers)
                                .filter(([_, answer]) => answer && answer !== 'false')
                                .map(([fieldId, answer]) => (
                                <div key={fieldId}>
                                    <label className="block text-sm font-medium text-slate-500">{customFieldsMap?.[fieldId] || fieldId}</label>
                                    <p className="mt-1 font-semibold text-slate-800 break-words">{answer === 'true' ? 'Yes' : String(answer)}</p>
                                </div>
                            ))}
                        </div>
                    </fieldset>
